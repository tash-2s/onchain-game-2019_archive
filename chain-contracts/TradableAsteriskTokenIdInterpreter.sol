...
library TradableAsteriskTokenIdInterpreter {
  ...
  function idToFields(uint256 tokenId)
    internal
    pure
  ...
  function fieldsToId(uint24 shortId, uint8 version, uint8 kind, uint8 paramRate, uint64 artSeed)
    internal
    pure
    returns (uint256)
  {
